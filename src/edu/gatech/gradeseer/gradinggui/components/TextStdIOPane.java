...
public class TextStdIOPane extends ToolbaredTextArea {
	...
	private TextStdIOPane(JTextArea area) {
		...
		JButton setToIn = new JButton("Set all text to stdin");
				...
				text.addKeyListener(new KeyAdapter() {
					...
					public void keyTyped(KeyEvent event) {
						try {
							...
							byte[] byteArray = txt.getBytes("UTF-8"); // choose
																		// a
		...
		JButton setCurrentToIn = new JButton("Set current text to stdin");
		setCurrentToIn.addActionListener(new ActionListener() {
		...
		super.addToolbarComponent(setCurrentToIn);
